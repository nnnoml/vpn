    ...
    <div style="padding: 15px;">
        ...
        <div class="layui-form">
            <table class="layui-table">
                ...
                <tr ac_id="{{$vo['ac_id']}}" ac_name="{{$vo['ac_name']}}" parent_id="{{$vo['parent_id']}}">
                    <td>{{$vo['ac_id']}}</td>
                    <td>{!! $vo['ac_name_format'] !!}</td>
                    <td>
        ...
        <div class="layui-form-item" id="parent_input_edit">
            ...
            <div class="layui-input-block">
                <select id="edit_parent_id" name="parent_id" lay-verify="required">
                    ...
                    @foreach ($list as $key=>$vo)
                        <option value="{{$vo['ac_id']}}">{!! $vo['ac_name_format']!!}</option>
                    @endforeach
                </select>
            </div>
        </div>
  ...
  layui.use(['form','layer','jquery'], function(){
      ...
      var layer = layui.layer;
        ...
        $(".addson").click(function(){
            cleanAlert();
            type='post';
            post_data['parent_id'] = $(this).parent().parent().attr('ac_id');
            $("#parent_input").show();
            $("#parent_input input").val($(this).parent().parent().attr('ac_name'));
            layer.open({
              type:1
              ,title: false
              ,area: ['450px', '220px']
        ...
        $(".edit").click(function(){
            cleanAlert();
            type='put';
            ac_id = $(this).parent().parent().attr('ac_id');
            post_data['parent_id'] = $(this).parent().parent().attr('parent_id');
            $("input[name='class_name']").val($(this).parent().parent().attr('ac_name'));
        ...
        $(".del").click(function(){
            ...
            layer.confirm('确认删除该组？ 旗下的分组也会同步删除', {
              btn: ['确定','取消'] //按钮
                ...
                var url = '/{{config('sys_conf.admin')}}/ArticleClass/'+ac_id;
        ...
        form.on('submit(article)',function () {
            ...
            if(type=='put'){
                ...
                var url = '/{{config('sys_conf.admin')}}/ArticleClass';
            }
